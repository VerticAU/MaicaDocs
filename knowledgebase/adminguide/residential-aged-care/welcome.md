...
{% hint style="info" %}
Setting the solution up for the first time? Start with [Prerequisites and Dependencies](prerequisites-and-dependencies.md), then [The RACS Configuration Tab](the-racs-configuration-tab/).
{% endhint %}
...
* **RACS Settings and Rate Configuration** covers the global rates, caps, indexation values, and automation toggles on the RACS Configuration tab. See [The RACS Configuration Tab](the-racs-configuration-tab/).
* **The Billing Engine** explains how the engine turns Agreement Items into invoices, the fee type rules, billing dates and catch-up chains, fee rate detection, indexation, and scheduling. See [Billing Engine Architecture](billing-engine-architecture/).
