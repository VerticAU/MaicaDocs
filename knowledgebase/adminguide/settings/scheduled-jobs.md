...
Then, the job will execute the Billing Flow selected in the Billing Management Settings that creates `Invoice Line Items` related to the `Delivery Activity`.\
\
...
{% hint style="info" %}
This job generates **standalone** recurring Shifts, meaning Shifts created from a Master Shift that do not belong to a Roster. Shifts that belong to a Roster are generated by the `Manage Recurring Rosters` job instead.
{% endhint %}
...
This job maintains recurring **Rosters**, keeping each approved Roster series generated forward to the Schedule Horizon on an ongoing basis. Approving a master Roster generates its series immediately; this job keeps the series rolling forward from that point.
...
* `End_Date__c` >= TODAY
* `Under_Evaluation__c` = TRUE
* `Master_Roster__r`.`Status__c` = 'Approved'

For each `Schedule` Record, the Job creates the Rosters that are **missing** between the current date and the Schedule Horizon determined in the General Settings. Rosters that already exist are skipped, so the job is safe to run repeatedly and will not duplicate a series.

Once the Rosters exist, a second background process materialises their `Shifts`. The two run as a chain, and progress across both is reported on a single progress indicator.

{% hint style="warning" %}
A series whose master Roster is still `Draft` is never processed by this job, even where its Schedule is otherwise eligible. If a Roster series has stopped generating, check the master Roster's status first.
{% endhint %}

{% hint style="info" %}
A user-initiated approve or reevaluate of a single master Roster runs the same logic scoped to that one Schedule, so it never regenerates another Schedule's Rosters. To learn more, see Recurring Schedules.
{% endhint %}
...
\
If the `Enforce Availability for Appointment` Setting under Validation Management is enabled, the Job will perform the same Validation as a user would creating the Appointment manually, hence, if `Resources` are `Unavailable` they will not be copied across to the newly created `Appointments`.\
\
Note: The `Original Schedule Start` field stores the value of the `Schedule Start` field when it is changed for the first time after it has been created.
...
\
The Status for each Record is being determined via the Validation Management Settings. **Maica** will default the Status to `Submitted`, but you can change this to suit your preference.\
\
...
{% hint style="info" %}
An occurrence that has been **Cancelled** is skipped by this job and is not recreated. To learn more, see [Manage Unavailability](https://app.gitbook.com/s/hehRshYIRk6XUlay9L3b/the-planner/planner-actions/manage-unavailability) in the User Guide.
{% endhint %}
