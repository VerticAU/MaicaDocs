...
To create a new Location for a Participant, click the **New Location** action on the Participant record and complete the Location details. **Maica** links the new Location to the Contact automatically, so it appears in this related list once saved.

{% hint style="info" %}
A Participant can have more than one Location. One of them can be marked as the **Primary Service Delivery Location**, which is the address **Maica** treats as their default when scheduling.
{% endhint %}

{% hint style="warning" %}
A Participant can only have one Primary Service Delivery Location at a time. If you mark a second Location as Primary directly on the Location record while another is already set, **Maica** shows an error naming the existing Primary Location, so you can find it and update it first.
{% endhint %}
