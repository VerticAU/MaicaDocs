...
<figure><img src="https://2670482622-files.gitbook.io/~/files/v0/b/gitbook-x-prod.appspot.com/o/spaces%2FhehRshYIRk6XUlay9L3b%2Fuploads%2FBy1oUTenaS5oyu08sMTa%2Fmanage%20unavailability.png?alt=media&#x26;token=9463f437-4fd9-4118-b173-4e5bbfc90825" alt="" width="563"><figcaption></figcaption></figure>
...
## Cancelling a Single Occurrence of a Recurring Unavailability

Previously, deleting a single occurrence of a Recurring Unavailability had no lasting effect: the nightly recurring-Unavailability batch could not tell a deliberately removed occurrence from one that simply had not been generated yet, and recreated it the following morning.

A single occurrence of a Recurring Unavailability can now be **Cancelled** instead of deleted, without affecting the rest of the series.

{% hint style="info" %}
A Cancelled occurrence is retained as a record (rather than deleted), is skipped by the nightly recurring-Unavailability batch, and no longer blocks the Resource from being assigned to Appointments or Shifts in that time slot. The rest of the recurrence series continues to generate and behave as normal.
{% endhint %}

{% hint style="warning" %}
The `Delete` quick action has been removed from the Unavailability Lightning page in favour of `Cancel`. To free up a single occurrence of a Recurring Unavailability, use `Cancel` rather than `Delete`.
{% endhint %}
...
| You cancel a single occurrence of a Recurring Unavailability | ✅ That occurrence is marked Cancelled and is not recreated by the nightly batch; the rest of the series continues as normal. |
